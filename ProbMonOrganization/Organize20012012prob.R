...
criticalLink <- read_csv('C:/HardDriveBackup/R/GitHub/ProbMon-Integrated-Reports/2020ProbChapter/processedData/Wadeable_ProbMon_2001-2018_Final_Final.csv') %>%
  # originally I used this one but when we realized how 3 sites in 2011 were super messed up for years
  # I fixed it with the above file. Use the above file forevermore
  #read_csv('C:/HardDriveBackup/R/GitHub/ProbMon-Integrated-Reports/2018/processedData/Wadeable_ProbMon_2001-2016_EVJ.csv')%>%
...
saveHere <- 'C:/HardDriveBackup/GIS/ProbMonGIS/DelineatedWatersheds/YearlyAnalyses/'
...
#           crs = 102008) %>% # NAD83 test
#  st_transform(crs = 4326)   # back to WGS84
#coordinates <- st_coordinates(eachYear)
#eachYear <- mutate(eachYear, LongitudeDD1 = coordinates[,1], LatitudeDD1 = coordinates[,2])
...
if(nrow(points) != nrow(watersheds) | nrow(eachYear) != nrow(watersheds)){
  missing <- unique(
...
watershedsQA <- filter(watershedsQA, StationID %in% c('1ACAH001.82','1BCPL002.83','2-RVN012.05',
                                                      '4AEKH003.18','4AHEN004.74','4ASRV012.19',
                                                      '5AAPW001.04','5ABLW055.26','6BPOW156.57')) # these are the only ones with actual issues
...
if(length(watershedsQA$StationID) >0){
  ...
  #watersheds <- filter(watersheds, !(UID %in% shapes))
