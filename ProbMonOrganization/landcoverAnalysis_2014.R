...
saveHere <- 'Results/ProbWadeable2013_2018/2014/new/'
...
#just 2014
wshdPolys <- st_read('C:/HardDriveBackup/GIS/ProbMonGIS/DelineatedWatersheds/YearlyAnalyses/2014_StreamStats/2014_StreamStats_watersheds.shp') %>%
  mutate(StationID = sub("\r\n" ,"",UID)) %>% # get rid of any stray spaces after StationID in attribute table
  dplyr::select(StationID) %>%
  st_transform(st_crs(pop2000)) 
# Bring in all sampled in 2014 to find which ones missing from above shapefile (trend sites)
x2014 <- read_csv('C:/HardDriveBackup/R/GitHub/ProbMon-Integrated-Reports/2018/processedData/Wadeable_ProbMon_2001-2016_EVJ.csv')%>%
  ...
  filter(YearSampled == 2014)

x2014missing <- filter(x2014, !StationID %in% wshdPolys$StationID)

if(nrow(x2014missing) > 0 ){
  ...
  trendWatersheds <- st_read('C:/HardDriveBackup/GIS/ProbMonGIS/DelineatedWatersheds/YearlyAnalyses/trend_StreamStats/trend_StreamStats_watersheds.shp') %>%
    ...
    filter(StationID %in% x2014missing$StationID) %>%
    dplyr::select(StationID)%>%
    st_transform(st_crs(pop2000))
  ...
  # anything missing???
  wshdPolys$StationID[!(wshdPolys$StationID %in% x2014$StationID )]
  # cool 
  rm(trendWatersheds);rm(x2014missing)
}


## Make sites object, best to do this from the actual data bc trend StationID's
if(!('StationID_Trend' %in% names(x2014))){
  wshdSites <- x2014 %>%
    mutate(StationID = sub("\r\n" ,"",StationID), # get rid of any stray spaces after StationID in attribute table
           StationID_Trend = case_when(str_detect(StationID_Trend, '_') ~ StationID_Trend,
  ...
  wshdSites <- dplyr::select(x2014, StationID, StationID_Trend, LatitudeDD, LongitudeDD) 
