...
saveHere <- 'Results/ProbWadeable2013_2018/2015/'
...
#just 2015
wshdPolys <- st_read('C:/HardDriveBackup/GIS/ProbMonGIS/DelineatedWatersheds/YearlyAnalyses/2015_StreamStats/2015_StreamStats_watersheds.shp') %>%
  mutate(StationID = sub("\r\n" ,"",UID)) %>% # get rid of any stray spaces after StationID in attribute table
  dplyr::select(StationID) %>%
  st_transform(st_crs(pop2000))
# Bring in all sampled in 2015 to find which ones missing from above shapefile (trend sites)
x2015 <- read_csv('C:/HardDriveBackup/R/GitHub/ProbMon-Integrated-Reports/2018/processedData/Wadeable_ProbMon_2001-2016_EVJ.csv')%>%
  ...
  filter(YearSampled == 2015)

x2015missing <- filter(x2015, !StationID %in% wshdPolys$StationID)

if(nrow(x2015missing) > 0 ){
  ...
  trendWatersheds <- st_read('C:/HardDriveBackup/GIS/ProbMonGIS/DelineatedWatersheds/YearlyAnalyses/trend_StreamStats/trend_StreamStats_watersheds.shp') %>%
    ...
    filter(StationID %in% x2015missing$StationID) %>%
    dplyr::select(StationID)%>%
    st_transform(st_crs(pop2000))
  ...
  # anything missing???
  wshdPolys$StationID[!(wshdPolys$StationID %in% x2015$StationID )]
  # cool 
  rm(trendWatersheds);rm(x2015missing)
}


## Make sites object, best to do this from the actual data bc trend StationID's
if(!('StationID_Trend' %in% names(x2015))){
  wshdSites <- x2015 %>%
  ...
  wshdSites <- dplyr::select(x2015, StationID, StationID_Trend, LatitudeDD, LongitudeDD) 
