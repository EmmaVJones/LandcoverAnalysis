...
wd <- "E:/evjones/GIS/ProbMonGIS/GISdata"

# Where do you want to save the outputs? 
saveHere <- 'Results/ProbWadeable20192020/rerun'
...
# Final 2019, 2020 station list, just targeted data for wadeable chapter
stations <- readxl::read_excel('C:/HardDriveBackup/R/GitHub/FreshwaterProbMonIntegratedReports/2022ProbChapter/originalData/Wadeable_ProbMon_2019-2020_Final.xlsx',
                     sheet = 'All20192020StationInfo') %>% 
    filter(status %in% c('TS')) %>% 
  dplyr::select(StationID:LatitudeDD)
  



# polygons
wshdPolys <- st_read('C:/HardDriveBackup/GIS/ProbMonGIS/DelineatedWatersheds/Watersheds.gdb', 'AllWatersheds_through2020') %>%
