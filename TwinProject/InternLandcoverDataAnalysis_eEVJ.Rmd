...
# bring in NLCD 2016
landcover <- #as.data.frame(
  raster("GISdata/mapData/nlcd2016.TIF")# )
...
autoShed_Polys <- filter(wshdPolys, StationID %in% humanShed_Poly$StationID) %>%
  arrange(StationID)


i =17 # potential sites to show 1, 5, 9, 10


humanShed_Poly[i,]$StationID
autoShed_Polys[i,]$StationID

landcover2 <- gplot_data(landcover)

ggplot() +  
  geom_tile(data=landcover, aes(x=x, y=y, fill=value), alpha=0.8)
  geom_sf(data = humanShed_Poly[i,], color= 'black', fill= NA) +
  geom_sf(data = autoShed_Polys[i,], color= 'red', fill= NA) + 
  theme_void()
